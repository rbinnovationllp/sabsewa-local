...
- If a verified vendor mobile matches a vendor application that is unclaimed or linked to an older auth user with the same verified phone, the backend may safely associate that vendor row with the current authenticated user through an audited resolver. Conflicting ownership must stop the flow and send the user to support/manual recovery rather than looping or granting access.
...
- Every vendor must complete neutral business verification before appearing to customers, including proprietor/entity name, public shop/trade name, business address, verified phone, authorised representative, category, PAN/GSTIN where applicable, FSSAI/drug/category licences where required, shop photos, location verification, accuracy declaration and mandatory business-establishment address/occupancy proof for the declared shop or branch premises.
- Business-establishment proof must identify the occupancy model (rented/leased, vendor-owned, family-owned, shared/licensed or other lawful occupancy), support the declared business address, flag material address mismatch for admin review, and treat recent utility bills as supporting evidence only. Utility bills should preferably be no older than 3 months, and documents in another person's name should be supported by tenancy, consent/NOC, relationship or authorization evidence where applicable.
...
- Company CRM must show Partner KYC and Vendor KYC as separate review monitors. Partner KYC counters are sourced from the secured backend Partner KYC summary API and Partner Applications can be opened directly with pending, approaching-deadline, overdue, approved, rejected and resubmission filters.
- Partner KYC submission confirmation must be based on successful database updates and must show the application reference number before telling the applicant that preliminary screening and manual verification are pending.
