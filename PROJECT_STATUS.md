...
- Follow-up fix: Vendor Login from the Home page now uses the explicit `/auth/Login?role=vendor&intent=vendor_login` route. If a Master Admin/Admin/customer/partner session is already active, the login page shows a secure switch-account warning and a `Sign out and continue as Vendor` action instead of auto-opening Company CRM.
- The Home page now uses the resolved AuthProvider role instead of raw `user_metadata.role`, preventing stale metadata from deciding whether customer/vendor buttons are shown.
