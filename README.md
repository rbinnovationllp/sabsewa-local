...
```text
SabSewa-Local/
├── backend/            Root backend service
├── mobile/             Expo React Native application (Web & Mobile)
│   ├── app/            Expo Router screens (auth, customer, vendor, company)
│   ├── components/     Shared UI components (PWA prompts, ProductGrid)
│   ├── lib/            Utility helpers (phone, secureStorage, deviceIdentity)
│   └── server/         Active SabSewa Local Express backend API
├── supabase/           Database schema, migrations, RLS policies, and functions
├── PRD/                Product requirements and specifications
├── docs/               Hackathon compliance, runbooks, and deployment guides
├── README.md           Project overview
└── PROJECT_STATUS.md   Live status, changelog, and readiness audits