...
- **Role-First Public Entry:** The public home page now starts with clear customer, vendor and partner entry choices so normal vendor/customer journeys do not accidentally fall into Company CRM or Master Admin verification.
- **Search-First Customer Dashboard:** Customer Home is organized around product search, category discovery, voice/text ordering, cart review, order tracking, profile, support and complaint actions while reusing the live discovery and cart modules.
...
- **Grouped Vendor Operations:** Vendor CRM actions are grouped into Start Here, Orders, Products & Availability, Delivery, and Money/Credit/Billing so one-person shops can operate simply while larger shops keep access to the full toolset.
