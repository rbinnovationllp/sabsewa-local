...
export default function GeminiOrderScreen() {
  ...
  async function useCurrentLocation() {
    ...
    setShopMessage("Location added. Tap Find Nearby Shops to search verified shops.");
  }
  ...
  async function findNearbyShops() {
    ...
    if (!lat && !lng && !pincode.trim() && !locality.trim()) {
      setShopMessage("Use your location or enter PIN/locality before searching.");
      return;
    }
    ...
    try {
      ...
      const filtered = term
        ? (json.vendors || []).filter((shop: any) => {
            const shopText = `${shop.shop_name || ""} ${shop.category || ""} ${shop.delivery_terms || ""}`.toLowerCase();
            const productText = (shop.available_products || [])
              .map((item: any) => `${item.item_name || ""} ${item.price_label || ""}`)
              .join(" ")
              .toLowerCase();
      ...
      setShopMessage(
        filtered.length > 0
          ? `Found ${filtered.length} verified nearby shop(s). Select one to continue.`
          : "No matching verified shop was found within 1 kilometre. Try another category, product name or locality."
      );
    } catch (error) {
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {!selectedShop ? (
        <View style={styles.panel}>
          ...
          <TextInput style={styles.input} value={searchText} onChangeText={setSearchText} placeholder="Shop name, category or product" />
          ...
          <TextInput style={styles.input} value={pincode} onChangeText={setPincode} placeholder="PIN code" keyboardType="number-pad" />
          <TextInput style={styles.input} value={locality} onChangeText={setLocality} placeholder="Locality" />
        ...
        <View style={styles.selectedPanel}>
          ...
          <Text style={styles.meta}>{[selectedShop.locality, selectedShop.distance_label].filter(Boolean).join(" | ") || "Nearby shop"}</Text>
          <Text style={styles.meta}>Status: {selectedShop.open_now ? "Open" : "Currently closed"} | Verified vendor</Text>
          <TouchableOpacity style={styles.secondaryBtn} onPress={() => setSelectedShop(null)}>
      ...
      {shops.map((shop) => (
        <TouchableOpacity key={`${shop.id}-${shop.terminal_id}`} style={styles.shopCard} onPress={() => setSelectedShop(shop)}>
          ...
          <Text style={styles.meta}>{shop.category} | {shop.distance_label || "Nearby"} | Verified</Text>
