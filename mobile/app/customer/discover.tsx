...
export default function CustomerVendorDiscoveryScreen() {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
        ...
        <View key={`${vendor.id}-${vendor.terminal_id}`} style={styles.vendorCard}>
          ...
          {(vendor.available_products || []).map((product: any) => (
            <Text key={product.id} style={styles.productLine}>
              {product.item_name} - {product.price_label || (product.price == null ? "Ask Vendor" : `Rs ${Number(product.price || 0).toFixed(2)}`)}
            </Text>
          ))}
      ...
      {!loading && searchRadius && vendors.length === 0 ? (
        <View style={styles.emptyCard}>
          <Text style={styles.emptyTitle}>We are sorry!</Text>
          <Text style={styles.emptyText}>
            SabSewa Local does not currently have a registered vendor for your selected requirement within 1 kilometre of your location.
          </Text>
          <Text style={styles.emptyText}>
            We have recorded your requirement. Our team will work to identify suitable local vendors in your area and invite them to join SabSewa Local so they can serve you digitally. We will notify you when the required service becomes available nearby.
          </Text>

          <TouchableOpacity style={styles.emptyBtn} onPress={() => saveUnservedLead("notify_me")} disabled={leadSaving}>
            <Text style={styles.emptyBtnText}>Notify Me When Available</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.emptyBtn} onPress={() => saveUnservedLead("request_local_vendor")} disabled={leadSaving}>
            <Text style={styles.emptyBtnText}>Request a Local Vendor</Text>
          </TouchableOpacity>
