-- SabSewa Local incremental migration bundle
-- Use this only after the initial/base migrations have already been applied.
...
begin
  ...
  values
    (v_wallet.id, p_vendor_id, 'payment_received', 5500, v_balance_before, v_balance_before, p_razorpay_payment_id, p_razorpay_order_id, p_razorpay_payment_id, p_razorpay_signature, false, p_payment_metadata || jsonb_build_object('accounting_entry', 'PAYMENT_RECEIVED')),
    (v_wallet.id, p_vendor_id, 'activation_fee', -500, v_balance_before, v_balance_before, p_razorpay_payment_id, p_razorpay_order_id, p_razorpay_payment_id, p_razorpay_signature, false, p_payment_metadata || jsonb_build_object('accounting_entry', 'NON_REFUNDABLE_ACTIVATION_FEE')),
    (v_wallet.id, p_vendor_id, 'security_deposit', 5000, v_balance_before, v_balance_after, p_razorpay_payment_id, p_razorpay_order_id, p_razorpay_payment_id, p_razorpay_signature, true, p_payment_metadata || jsonb_build_object('accounting_entry', 'REFUNDABLE_WALLET_CREDIT'));
