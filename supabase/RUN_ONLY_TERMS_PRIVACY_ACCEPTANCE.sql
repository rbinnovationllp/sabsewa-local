-- SabSewa Local: run only the Terms/Privacy/legal acceptance migration.
-- Use this on an existing Supabase project where the base SabSewa Local tables
-- and policies already exist.
--
-- Do not run RUN_ALL_MIGRATIONS_FOR_SABSEWA_LOCAL.sql on a non-blank database;
-- it contains base policies such as "Approved vendors are public readable" that
-- may already exist.
...
select
  'SabSewa Local legal acceptance migration completed' as status,
  now() as completed_at;
